...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // AssemblyAI LeMUR API Routes
  
  // Generate summary using LeMUR
  app.post("/api/transcriptions/:id/lemur/summary", requireAuth, async (req: AuthenticatedRequest, res) => {
    try {
      const id = parseInt(req.params.id);
      const transcription = await storage.getTranscription(id);
      ...
      if (transcription.status !== 'completed') {
        return res.status(400).json({ message: "轉錄尚未完成，無法進行摘要分析" });
      }

      if (!transcription.assemblyaiId) {
        return res.status(400).json({ message: "缺少 AssemblyAI 轉錄 ID" });
      }

      const lemur = new AssemblyAILemur(process.env.ASSEMBLYAI_API_KEY!);
      const summary = await lemur.summarizeTranscript({
        transcript_ids: [transcription.assemblyaiId],
        answer_format: req.body.format || "詳細且結構化的重點摘要",
        context: req.body.context || "這是一段音頻對話的轉錄內容"
      });

      res.json({ summary });
    } catch (error) {
      console.error("LeMUR summary error:", error);
      res.status(500).json({ 
        message: error instanceof Error ? error.message : "摘要生成失敗" 
      });
    }
  });

  // Ask questions using LeMUR
  app.post("/api/transcriptions/:id/lemur/questions", requireAuth, async (req: AuthenticatedRequest, res) => {
    try {
      const id = parseInt(req.params.id);
      const { questions } = req.body;

      if (!questions || !Array.isArray(questions)) {
        return res.status(400).json({ message: "請提供問題清單" });
      }
      ...
      if (transcription.status !== 'completed') {
        return res.status(400).json({ message: "轉錄尚未完成，無法進行問答分析" });
      }
      ...
      const lemur = new AssemblyAILemur(process.env.ASSEMBLYAI_API_KEY!);
      const answers = await lemur.askQuestions({
        transcript_ids: [transcription.assemblyaiId],
        questions: questions,
        context: req.body.context || "這是一段音頻對話的轉錄內容"
      });
      ...
      res.status(500).json({ 
        message: error instanceof Error ? error.message : "問答分析失敗" 
      });
    }
  });

  // Generate meeting insights using LeMUR
  app.post("/api/transcriptions/:id/lemur/meeting-insights", requireAuth, async (req: AuthenticatedRequest, res) => {
    try {
      const id = parseInt(req.params.id);
      const transcription = await storage.getTranscription(id);
      ...
      if (transcription.status !== 'completed') {
        return res.status(400).json({ message: "轉錄尚未完成，無法進行會議分析" });
      }
      ...
      const lemur = new AssemblyAILemur(process.env.ASSEMBLYAI_API_KEY!);
      const insights = await lemur.generateMeetingInsights([transcription.assemblyaiId]);

      res.json(insights);
    } catch (error) {
      console.error("LeMUR meeting insights error:", error);
      res.status(500).json({ 
        message: error instanceof Error ? error.message : "會議分析失敗" 
      });
    }
  });

  // Generate interview analysis using LeMUR
  app.post("/api/transcriptions/:id/lemur/interview-analysis", requireAuth, async (req: AuthenticatedRequest, res) => {
    try {
      const id = parseInt(req.params.id);
      const transcription = await storage.getTranscription(id);
      ...
      if (transcription.status !== 'completed') {
        return res.status(400).json({ message: "轉錄尚未完成，無法進行面試分析" });
      }
      ...
      const lemur = new AssemblyAILemur(process.env.ASSEMBLYAI_API_KEY!);
      const analysis = await lemur.generateInterviewAnalysis([transcription.assemblyaiId]);

      res.json(analysis);
    } catch (error) {
      console.error("LeMUR interview analysis error:", error);
      res.status(500).json({ 
        message: error instanceof Error ? error.message : "面試分析失敗" 
      });
    }
  });

  // Generate customer service analysis using LeMUR
  app.post("/api/transcriptions/:id/lemur/customer-service-analysis", requireAuth, async (req: AuthenticatedRequest, res) => {
    try {
      const id = parseInt(req.params.id);
      const transcription = await storage.getTranscription(id);
      ...
      if (transcription.status !== 'completed') {
        return res.status(400).json({ message: "轉錄尚未完成，無法進行客服分析" });
      }
      ...
      const lemur = new AssemblyAILemur(process.env.ASSEMBLYAI_API_KEY!);
      const analysis = await lemur.generateCustomerServiceAnalysis([transcription.assemblyaiId]);

      res.json(analysis);
    } catch (error) {
      console.error("LeMUR customer service analysis error:", error);
      res.status(500).json({ 
        message: error instanceof Error ? error.message : "客服分析失敗" 
      });
    }
  });
